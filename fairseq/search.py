...
class LexicallyConstrainedBeamSearch(Search):
    ...
    def step(
        ...
        if not self.constraint_states:
            ...
            # Prevent EOS for unfinished hyps across all batch items
            if step > 0:
                not_finished_indices = []
                for sentno, sent_constraints in enumerate(self.constraint_states):
            ...
            indices_buf = torch.full((batch_size, 2 * beam_size), self.pad, device=device, dtype=torch.long)
            scores_buf = torch.full((batch_size, 2 * beam_size), -1e6, device=device)
            beams_buf = torch.full((batch_size, 2 * beam_size), beam_size - 1, device=device, dtype=torch.long)
            ...
            for sentno, sent_constraints in enumerate(self.constraint_states):
                ...
                for beamno, state in enumerate(sent_constraints):
                    if state is None:
                        continue
                    next_tokens, next_states = state.next_legals()
                    if not next_tokens:
                        next_tokens = [self.eos]
                        next_states = [None]
                    next_tokens_buf.extend(next_tokens)
                    next_beams_buf.extend([beamno for _ in next_tokens])
                    next_states_buf.extend(next_states)
                    next_scores_buf.append(lprobs[sentno, beamno][next_tokens])
                    if step == 0:
                        # only use first beam for each sentence
                        break
                if not next_beams_buf:
                    # no valid next tokens, simply fill candidate next_tokens to eos and exit.
                    # beams and indice
                    indices_buf[sentno].fill_(self.eos)
                    continue
                next_tokens_buf = torch.tensor(next_tokens_buf, device=lprobs.device, dtype=torch.long)
                next_beams_buf = torch.tensor(next_beams_buf, device=lprobs.device, dtype=torch.long)
                next_scores_buf = torch.cat(next_scores_buf, dim=0)

                sort_values, sort_indices = next_scores_buf.sort(dim=0, descending=True)
                sort_indices = sort_indices[:self.num_cands]

                next_tokens_buf = next_tokens_buf[sort_indices]
                next_beams_buf = next_beams_buf[sort_indices]
                next_scores_buf = next_scores_buf[sort_indices]
                next_states_buf = [next_states_buf[int(i)] for i in sort_indices]

                num_cands = len(next_states_buf)
                indices_buf[sentno, :num_cands] = next_tokens_buf
                beams_buf[sentno, :num_cands] = next_beams_buf
                scores_buf[sentno, :num_cands] = next_scores_buf
                # placeholder needed for candidate selection in sequence generator
                # None states are skipped in processing
                self.constraint_states[sentno] = next_states_buf + [None for _ in range(2 * beam_size - num_cands)]
        return scores_buf, indices_buf, beams_buf
